...
public:
    ...
    // Sets the nominal alpha level (max type I error rate)
    void setAlpha(double a);
    // Set the maximal type II error rate
    void setBeta(double b);
    // Sets the probability of an event under the null hypothesis
    void setP0(double p0);
    // Sets the probability of an event under the alternativ hypothesis
    void setP1(double p1);
    // Sets the probability of an event in the supset endpoint under the null hypothesis
    ...
    // Sets the probability of an event in the supset endpoint under the alternativ hypothesis
    void setPc1(double pc1);
    // Sets the probability of an event in the superset endpoint under the alternativ hypothesis
    void setPt1(double pt1);
    // Aproximates "maxN" (maximal number of patients to be recruited) in a way that it is most likly to identify the optimal design.
    ...
    // Calculates the pobability for a type one error    
    // n1: sample size for the first stage.
    // r1: critical value for the first stage.
    ...
    // Calculates the pobability for a type two error
    // n1: sample size for the first stage.
    // r1: critical value for the first stage.
    // n: overall sample size.
    // r: critical value for the subset endpoint.
    ...
    // Retuns the conditional power given that there are "t" responses in the supset endpoint and "u" responses in the superset endpoint.
    // t: observed responses in the subset endpoint.
    // u: observed responses in the superset endpoint.
    ...
    // reps: Number of simulations to estimate the effect of (non-)stochastic curtailment
    ...
    //Returns the ID of the minimax design under the found solutionstrategies. (only used by the GUI for the R-package)
    int getMinimaxPos();
    //Returns the ID of the optimal design under the found solutionstrategies. (only used by the GUI for the R-package)
    int getOptimalPos();
...
private:
    ...
    //returns ture if a design was found
    ...
    // PH.D. thesis, Medical Faculty of Heidelberg, Rupechts-Karls_universitaet" on page 16.
